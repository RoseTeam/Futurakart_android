...
public class MultiTouchController<T> {
	...
	/**
	 * The biggest possible abs val of the change in mX or mY between multitouch events (larger dx/dy events are ignored) -- helps eliminate jumps in
	 * pointer position on finger 2 up/down.
	 */
	private static final float MAX_MULTITOUCH_POS_JUMP_SIZE = 300.0f;
	 ...
	 */
	private static final float MAX_MULTITOUCH_DIM_JUMP_SIZE = 400.0f;
	...
	static {
		...
		try {
			...
			e.printStackTrace();
			Log.e("MultiTouchController", "static initializer failed");
		}
		...
		try {
			...
			for (int histIdx = 0; histIdx <= histLen; histIdx++) {
				...
				if (!multiTouchSupported || pointerCount == 1) {
					...
					// Read mX, mY and pressure of each pointer
					if (DEBUG)
			...
			e.printStackTrace();
			Log.e("MultiTouchController", "onTouchEvent() failed");
			return false;
		}
	}
	...
	public static class PointInfo {
		...
		/** Difference between mX coords of touchpoint 0 and 1. */
		public float getMultiTouchWidth() {
			return isMultiTouch ? dx : 0.0f;
		}

		/** Difference between mY coords of touchpoint 0 and 1. */
		...
		/**
		 * Calculate the angle of a multitouch event, and cache it. Actually gives the smaller of the two angles between the mX axis and the line
